#!/usr/bin/env python3
"""
👑 QUILLAN-RONIN v5.3.1 — HIGH-PRECISION MULTI-FORMAT MASTER SFT ENGINE
Trains the 12-layer unrolled 34-expert sovereign model on 3,200 target-masked multi-format pairs
using calibrated AdamW(2e-4) with bounded MoE modulation and frequency/presence penalties.
"""
...
print("==================================================================", flush=True)
print("   👑 HIGH-PRECISION MULTI-FORMAT MASTER SFT ENGINE", flush=True)
print("   [3,200 Samples | 34 Experts | Bounded MoE | AdamW(2e-4)]", flush=True)
print("==================================================================\n", flush=True)

enc = tiktoken.get_encoding("gpt2")

# 1. Load Multi-Format Target-Masked Dataset
...
print(f"[+] Loaded {num_samples:,} target-masked samples (SeqLen={MAX_SEQ_LEN})!\n", flush=True)
...
def eval_milestone(step_num):
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  [SFT MILESTONE EVALUATION @ STEP {step_num}]", flush=True)
    print(f"{'='*65}", flush=True)
    for cat, prompt in test_queries:
        toks = enc.encode(prompt)
        t0 = time.time()
        out = model.generate(toks, max_tokens=55, temp=0.7, top_k=50, top_p=0.90, repetition_penalty=1.05, frequency_penalty=0.5, presence_penalty=0.4)
...
print(f"[TRAIN] Launching Master SFT ({STEPS} steps, LR={BASE_LR} -> {MIN_LR})...\n", flush=True)
...
torch.save({
    ...
    'step': STEPS,
    'loss': best_loss,
    'version': 'quillan-v5.3.1-unrolled-gold-master-final'
}, ckpt_path)

print(f"\n[DONE] 🏆 Master SFT Complete! Best Target Loss: {best_loss:.4f} in {(time.time()-t_train)/60:.1f}m\n", flush=True)
...
for idx, (cat, prompt) in enumerate(BENCHMARK_QUESTIONS, 1):
    toks = enc.encode(prompt)
    t_start = time.time()
    out = model.generate(toks, max_tokens=60, temp=0.6, top_k=40, top_p=0.85, repetition_penalty=1.05, frequency_penalty=0.6, presence_penalty=0.4)
