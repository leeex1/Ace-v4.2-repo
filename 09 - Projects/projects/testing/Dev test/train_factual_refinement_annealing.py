#!/usr/bin/env python3
"""
👑 QUILLAN-RONIN v5.3.1 — FACTUAL REFINEMENT & ANNEALING ENGINE
Fine-tunes the solid unrolled 34-expert sovereign model on exact domain facts
using gentle AdamW (1.5e-5 -> 3e-7) with weight decay (0.08) and gradient clipping (0.3).
"""
...
print("==================================================================", flush=True)
print("   👑 FACTUAL REFINEMENT & ANNEALING ENGINE", flush=True)
print("   [300 Steps | LR=1.5e-5 -> 3e-7 | Weight Decay=0.08 | 34 Experts]", flush=True)
print("==================================================================\n", flush=True)

enc = tiktoken.get_encoding("gpt2")

# 1. Load Solid Base Checkpoint
print(f"[*] Loading solid base checkpoint: {ckpt_path.name}...", flush=True)
ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
model = QuillanUnrolledSovereign(ckpt["cfg"]).to("cpu")
model.load_state_dict(ckpt["model_state_dict"])
print("[+] Model loaded successfully!\n", flush=True)

# 2. Load Target-Masked Dataset
...
# 3. Trainable Parameters Isolation (Freeze dense backbone)
adapter_params = []
...
optimizer = torch.optim.AdamW(adapter_params, lr=BASE_LR, weight_decay=0.08)
...
def eval_milestone(step_num):
    ...
    print(f"\n{'='*65}", flush=True)
    print(f"  [REFINEMENT MILESTONE EVALUATION @ STEP {step_num}]", flush=True)
    print(f"{'='*65}", flush=True)
    for cat, prompt in test_queries:
        toks = enc.encode(prompt)
        t0 = time.time()
        out = model.generate(toks, max_tokens=45, temp=0.6, top_k=40, top_p=0.85, repetition_penalty=1.05)
...
print(f"[TRAIN] Launching Factual Refinement ({STEPS} steps, LR={BASE_LR} -> {MIN_LR})...\n", flush=True)
...
for step in range(1, STEPS + 1):
    ...
    torch.nn.utils.clip_grad_norm_(adapter_params, 0.3)
    optimizer.step()
    ...
    if step % 25 == 0 or step == 1:
        ...
        print(f"  step {step:3d}/{STEPS}  target_loss={val:.4f}  best={best_loss:.4f}  lr={lr:.8f}  ({sps:.2f}s/st, ETA {eta_m:.1f}m)", flush=True)
    ...
    if step % 100 == 0:
        ...
        torch.save({
            ...
            'step': step,
            'loss': best_loss,
            'version': 'quillan-v5.3.1-unrolled-refined-master'
        }, ckpt_path)
        print(f"  [CHECKPOINT] Auto-saved refined master checkpoint at step {step}.\n", flush=True)
...
torch.save({
    ...
    'step': STEPS,
    'loss': best_loss,
    'version': 'quillan-v5.3.1-unrolled-refined-master-final'
}, ckpt_path)

print(f"\n[DONE] 🏆 Factual Refinement Complete! Best Target Loss: {best_loss:.4f} in {(time.time()-t_train)/60:.1f}m\n", flush=True)

# 10-Question Benchmark Suite
print("==================================================================", flush=True)
...
for idx, (cat, prompt) in enumerate(BENCHMARK_QUESTIONS, 1):
    toks = enc.encode(prompt)
    t_start = time.time()
    out = model.generate(toks, max_tokens=55, temp=0.6, top_k=40, top_p=0.85, repetition_penalty=1.05)
