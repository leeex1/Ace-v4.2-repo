#!/usr/bin/env python3
"""
Quillan-Ronin v5.3.1 — Deep All-Dataset Conversational Alignment SFT Script
Combines ALL training datasets in training_data/ into a unified target-masked conversational corpus.
Supports 'messages', 'text', 'prompt/response', and 'conversations' JSON schema formats.
Enforces strict target-masking on System/User tokens (-100) and trains 100% of autograd capacity on Assistant dialogue responses.
"""
                ...
                try:
                    ...
                    for msg in messages:
                        ...
                        if role in ['system', 'user']:
                            # Mask system/user prompt tokens in loss calculation
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="Quillan Deep All-Dataset Conversational Alignment")
    parser.add_argument("--steps", type=int, default=5000, help="Number of SFT steps")
    parser.add_argument("--lr", type=float, default=2.5e-4, help="Learning rate")
    parser.add_argument("--seq-len", type=int, default=512, help="Sequence length")
    args = parser.parse_args()

    print("=" * 65)
    print("  QUILLAN-RONIN v5.3.1 — DEEP ALL-DATASET CONVERSATIONAL SFT PASS")
    ...
    print(f"\n[TRAIN] Starting {args.steps}-step Deep All-Dataset Conversational Alignment...", flush=True)
    t0 = time.time()
    ema_loss = None
    ...
    print("\n" + "=" * 65)
    print("  DEEP ALL-DATASET CONVERSATIONAL ALIGNMENT COMPLETE")
