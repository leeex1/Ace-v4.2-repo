...
# -*- coding: utf-8 -*-
"""
👑 QUILLAN-RONIN v5.3.1 — HIGH-PRECISION GOLD REASONING ANNEALER
---------------------------------------------------------------------------------------
Executes targeted deep precision alignment directly on the Gold Reasoning Corpus
(1,504 verified QA pairs, architecture flowcharts, formulas, and System 2 CoT solutions).

Drives loss from 2.23 down to <1.20 to lock in exact factual accuracy,
Python code generation, and logical deductions.
"""
...
LOGGER = logging.getLogger("precision_annealer")

def load_gold_dataset(tokenizer: SovereignTokenizer, max_seq_len: int = 384) -> List[Tuple[torch.Tensor, torch.Tensor]]:
    """Loads and tokenizes Gold Reasoning dataset with exact target masking."""
    gold_file = TRAIN_DATA_DIR / "Quillan_Clean_Reasoning_Gold_Dataset.jsonl"
    LOGGER.info("Loading gold reasoning corpus from %s...", gold_file.name)
    ...
    with open(gold_file, "r", encoding="utf-8", errors="ignore") as f:
        for line in f:
            ...
            num_targets = sum(1 for x in labels if x != -100)
            if len(full_sequence) > 1 and num_targets > 0:
                inp_t = torch.tensor(full_sequence, dtype=torch.long)
                lbl_t = torch.tensor(labels, dtype=torch.long)
                samples.append((inp_t, lbl_t))
                
    LOGGER.info("Loaded %d high-precision gold samples.", len(samples))
    return samples

def main():
    LOGGER.info("==================================================================")
    LOGGER.info("   👑 GOLD REASONING DEEP PRECISION ANNEALING (LOSS < 1.20)")
    LOGGER.info("==================================================================")
    ...
    aligned_best_path = CKPT_DIR / "quillan_34_experts_aligned_best.pt"
    precision_best_path = CKPT_DIR / "quillan_gold_precision_best.pt"

    model = QuillanUnrolledSovereign(cfg).to(device)

    # Resume from the 2.23 loss best checkpoint
    resume_path = aligned_best_path if aligned_best_path.exists() else master_ckpt_path
    ...
    model.train()
    dataset = load_gold_dataset(tokenizer, max_seq_len=384)
    ...
    LOGGER.info("Starting Precision Annealing Pass (%d steps, Peak LR: %.2e)...", total_steps, peak_lr)
    ...
    for step in range(1, total_steps + 1):
        ...
        if step % 50 == 0:
            ...
            if avg_loss < best_loss:
                best_loss = avg_loss
                LOGGER.info("--> Saving new best precision checkpoint (Loss: %.4f) to %s", best_loss, precision_best_path.name)
                torch.save({
                    "step": step,
                    "model_state_dict": model.state_dict(),
                    "loss": best_loss,
                }, str(precision_best_path))

        if step % 250 == 0:
            LOGGER.info("--> Periodic master snapshot at step %d to %s", step, master_ckpt_path.name)
            torch.save({
                "step": step,
                "model_state_dict": model.state_dict(),
                "loss": best_loss,
    ...
    LOGGER.info("==================================================================")
    LOGGER.info("   🏆 PRECISION ANNEALING COMPLETE (Best Loss: %.4f)", best_loss)
    LOGGER.info("==================================================================")
