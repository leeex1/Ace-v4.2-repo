...
log_test("Formula 11 (ASZR Adèlic Spectral Zeta Regularizer)", aszr.item() >= 0.0, f"Silver Ratio Gap Loss: {aszr.item():.6f}")
...
log_test("ASZR Loss in Model Aux Losses", "aszr" in aux_dict, f"ASZR: {aux_dict.get('aszr', torch.tensor(0.0)).item():.6f}")
