...
print("==================================================================")
print("   👑 QUILLAN-RONIN v5.3.1 — GENERAL KNOWLEDGE SFT ALIGNMENT")
print("==================================================================")
...
# Load both Hyper-Tune Gold and General Knowledge Datasets
dataset_paths = [
...
print(f"[DATA] Loaded {len(samples)} total target-masked samples across all domains (SeqLen=448).")
...
print(f"\n[TRAIN] Launching {STEPS} General Knowledge SFT Alignment Steps (LR={BASE_LR} -> {MIN_LR})...")
...
final_save_path = r"C:\02_QUILLAN\checkpoints\checkpoints_sft\quillan_omni_general_v531.pt"
torch.save({
    'model_state_dict': model.state_dict(),
    'step': STEPS,
    'loss': best_loss,
    'version': 'quillan-v5.3.1-omni-general'
}, final_save_path)
print(f"\n[SAVE] Omni-General Master Checkpoint saved to: {final_save_path}")

# Run Unrelated Domains Verification
model.eval()

def generate_omni_response(domain, question, max_tokens=220, temp=0.2, top_p=0.9):
    prompt_txt = f"<|system|>\n# 🤖🧠 Quillan System Start 🧠🤖\n<|user|>\n{question}\n<|assistant|>\n"
...
print("\n=== UNRELATED DOMAINS VERIFICATION ===")
test_questions = [
...
for domain, q in test_questions:
    generate_omni_response(domain, q, max_tokens=220, temp=0.2)
