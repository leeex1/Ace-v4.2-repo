#!/usr/bin/env python3
"""
👑 QUILLAN-RONIN v5.3.1 — 100% FULL-STACK END-TO-END UNIFIED TRAINER
Zero frozen layers. Trains every single parameter across Ingestion, Prism, Diffusion, MoE, Swarms, Finalizers, and Decoder.
"""
...
print("==================================================================")
print("   👑 QUILLAN-RONIN v5.3.1 — 100% FULL-PARAMETER UNIFIED SFT")
print("==================================================================")
...
print(f"[*] Loading master checkpoint: {ckpt_path.name}")
ckpt = torch.load(ckpt_path, map_location="cpu", weights_only=False)
sd = ckpt.get("model_state_dict", ckpt)
model.load_state_dict(sd, strict=False)
print(f"[+] Loaded successfully.")

# Explicitly ensure 100% of all parameters are active
total_params = 0
for name, param in model.named_parameters():
    param.requires_grad = True
    total_params += param.numel()

print(f"[VERIFY] 100% Full-Stack Active: {total_params / 1e6:.1f}M Trainable Parameters (0 Frozen)")
...
dataset_configs = [
    ...
    ("Quillan_Hyper_Tune_Gold_Dataset.jsonl", 5),
    ("Quillan_Explanatory_Prose_Dataset.jsonl", 5),
    ("Quillan_General_Knowledge_Dataset.jsonl", 5),
    ("code_train.jsonl", 1),
...
print(f"[DATA] Total High-Density Multi-Domain Corpus: {len(training_samples)} samples\n")
...
def quick_eval(step_num):
    ...
    print(f"\n  [EVAL @ step {step_num}] '{q}' -> {out.strip()[:180]}\n", flush=True)
    model.train()

print(f"[TRAIN] Executing 100% Full-Stack Joint Training ({STEPS} steps, LR={BASE_LR} -> {MIN_LR})...\n")
...
for step in range(1, STEPS + 1):
    ...
    if step % 10 == 0 or step == 1:
        ...
        print(f"  step {step:3d}/{STEPS}  loss={val:.4f}  best={best_loss:.4f}  bypass={bypass_val:.3f}  ({sps:.1f}s/st, ETA {eta_m:.1f}m)", flush=True)
    ...
    if step % 50 == 0:
        ...
        torch.save({
            ...
            'version': 'quillan-v5.3.1-100percent-full-stack'
        }, ckpt_path)
        print(f"  [SAVE] Checkpoint synced to master at step {step}.", flush=True)

# Final Save
torch.save({
    'model_state_dict': model.state_dict(),
    'step': STEPS, 'loss': best_loss,
    'version': 'quillan-v5.3.1-100percent-master'
}, ckpt_path)

print(f"\n[DONE] 🏆 100% Full-Stack Training Complete! Best Loss: {best_loss:.4f}")
quick_eval(STEPS)
