...
print("==================================================================")
print("   👑 QUILLAN-RONIN v5.3.1 — CLEAN TARGET-MASKED REASONING SFT")
print("==================================================================")
...
# Load Clean Target-Masked Reasoning Dataset
dataset_path = REPO_ROOT / "training_data" / "Quillan_Clean_Reasoning_Gold_Dataset.jsonl"
samples = []
    ...
    with open(dataset_path, "r", encoding="utf-8") as f:
        for line in f:
            ...
            if q and ans:
                ...
                # CRITICAL: Target Response Masking (-100 on prompt tokens)
                labs = [-100] * len(p_toks) + toks[len(p_toks):]
                samples.append((toks[:512], labs[:512]))

print(f"[DATA] Ingestion complete! Total active target-masked samples: {len(samples)} (SeqLen=512).")
...
print(f"\n[TRAIN] Launching {STEPS}-Step Clean Target-Masked SFT Pass (LR={BASE_LR} -> {MIN_LR})...")
...
def save_checkpoint_safely(model, step, loss, path):
    ...
    try:
        torch.save({
            ...
            'step': step,
            'loss': loss,
            'version': 'quillan-v5.3.1-clean-target-masked-master'
        }, tmp_path)
...
print(f"\n[SAVE] Clean Target-Masked Master Checkpoint saved to: {final_save_path}")
