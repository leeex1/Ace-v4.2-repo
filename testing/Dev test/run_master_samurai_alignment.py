...
print("==================================================================")
print("   👑 QUILLAN-RONIN v5.3.1 — MASTER SAMURAI ALIGNMENT SFT")
print("==================================================================")
...
init_ckpt_path = r"C:\02_QUILLAN\checkpoints\checkpoints_sft\quillan_1000000x_master_init.pt"
print(f"[*] Loading Master Backbone Initialized Checkpoint: {init_ckpt_path}")
ckpt = torch.load(init_ckpt_path, map_location="cpu", weights_only=False)
sd = ckpt.get("model_state_dict", ckpt)
model.load_state_dict(sd)
print("[+] Successfully loaded 510/510 initialized layers.")

# Prepare Target-Masked Samurai Seed & Distilled SFT Samples
samples = []
...
# Multiply Samurai seed dataset samples to anchor identity
samples = samples * 5
...
print(f"[DATA] Loaded {len(samples)} clean target-masked Samurai SFT samples.")
...
print(f"\n[TRAIN] Running {STEPS} Master Samurai Alignment Steps (LR={LR})...")
...
for step in range(1, STEPS + 1):
    ...
    if step % 5 == 0 or step == 1:
        ...
        sps = elapsed / step
        print(f"  step {step:3d}/{STEPS}  loss={total_loss.item():.4f}  best={best_loss:.4f}  ({sps:.1f}s/st)", flush=True)

save_path = r"C:\02_QUILLAN\checkpoints\checkpoints_sft\quillan_1000000x_master_aligned.pt"
torch.save({
    'model_state_dict': model.state_dict(),
    'step': STEPS,
    'loss': best_loss,
    'version': 'quillan-v5.3.1-1000000x-master-aligned'
}, save_path)
print(f"\n[SAVE] Master Aligned Checkpoint saved to: {save_path}")

# Test Speech Generation
model.eval()

def generate_master_speech(prompt, max_tokens=120, temp=0.7, top_p=0.9):
...
print("\n=== MASTER ALIGNED 1,000,000X SPEECH DEMO ===")
prompts = [
    "<|user|>\nHello! Who are you?\n<|assistant|>\n",
...
for p in prompts:
    generate_master_speech(p, max_tokens=100, temp=0.7, top_p=0.9)
