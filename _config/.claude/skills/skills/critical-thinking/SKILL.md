...
tags: [reasoning, analysis, critical-thinking, decision-making, logic, problem-solving]
council: [C7-LOGOS, C17-NULLION, C18-SHEPHERD, C8-METASYNTH, C1-ASTRA]
...
Critical thinking is not linear — it is **iterative, adversarial, and self-correcting**. The best frontier models all share one meta-pattern: **they argue with themselves before committing**.

This skill operationalizes that into a structured, repeatable protocol designed for both autonomous reasoning and human-AI collaboration.

---

## 🔬 The 8-Phase Thinking Protocol (Enhanced)
...
| Complexity Level | Phases to Run | Estimated Time | Output Quality |
|---|---|---|---|
| Trivial (e.g., "What is 2+2?") | 1, 7 | < 30s | ✅ Sufficient |
| Simple (e.g., fact lookup) | 1, 2, 7 | ~1 min | ✅ Good |
| Moderate (e.g., comparison) | 1, 2, 3, 7 | ~3 min | ✅ Solid |
| Complex (e.g., problem diagnosis) | 1, 2, 3, 4, 5, 7 | ~5 min | ✅ Strong |
| High-Stakes (e.g., strategic decision) | All 8 phases | ~10+ min | ✅ Maximum |
| Paradox / Ethical Dilemma | 1, 3, 4, 6, 7, 8 | ~8 min | ✅ Deep |
...
- What would make this problem trivial? (inversion heuristic)

> 💡 *DeepSeek R1 pattern*: Always re-read the problem statement. Half of errors come from solving the wrong problem.

**Sub-phase: Assumption Inventory**

```
ASSUMPTION INVENTORY:
A1: [assumption] — Impact if wrong: ___ — Likelihood of being wrong: ___
A2: [assumption] — Impact if wrong: ___ — Likelihood of being wrong: ___
A3: [assumption] — Impact if wrong: ___ — Likelihood of being wrong: ___
```
...
- Note dependencies explicitly with dependency graph notation
...
- If a sub-problem seems too large, decompose it further recursively

**Dependency Matrix:**

| Sub-problem | Depends On | Unlocks | Can Parallelize |
|---|---|---|---|
| A | — | B | Yes |
| B | A | C | No |
| C | B | — | No |
...
| Hypothesis | Plausibility | Evidence Strength | Novelty | Risk if Wrong |
|---|---|---|---|---|
| H1 | High | Strong | Low | Low |
| H2 | Medium | Moderate | High | Medium |
| H3 | Low | Weak | Medium | High |
| H4 | Medium-High | Mixed | High | Low |
...
|---|---|---|
| **Red Team** | Actively try to disprove your own conclusion | Always |
| **Pre-mortem** | Assume the answer failed; explain why | High-stakes decisions |
| **Straw Man Fallacy Check** | Ensure you're not attacking a weak version of the counterargument | Debates, evaluations |
| **Outside View** | How often do similar claims turn out to be wrong? (base rates) | Predictions |
| **Inversion** | What if you had to achieve the OPPOSITE outcome? | Strategy, planning |
...
| Iteration | Hypothesis | Flaw Discovered | Revised To |
|---|---|---|---|
| 1 | H1 | [missing evidence] | H2 |
| 2 | H2 | [logical gap] | H4 |
| 3 | H4 | — (validated) | Final |
...
**Evidence Quality Grading (Distilled from Multiple Models):**

| Grade | Meaning | Example |
|---|---|---|
| **A** | Proven beyond reasonable doubt | Mathematical proof, replicated experiment |
| **B** | Strong evidence, limited counterexamples | Well-established theory, meta-analysis |
| **C** | Moderate evidence, meaningful uncertainty | Single study, expert consensus |
| **D** | Weak evidence, primarily speculative | Anecdote, first principles reasoning |
| **F** | No reliable evidence or contradicts known facts | Debunked claim, no sources |

**Confidence Calibration Rules (Distilled from Multiple Models):**
- 90%+ = Logical certainty or extremely well-established fact (Grade A)
- 70-89% = Strong evidence, no major known counterexamples (Grade B)
- 50-69% = Best available answer but meaningful uncertainty exists (Grade C)
- 25-49% = Speculative, explore more before committing (Grade D)
- Below 25% = Insufficient evidence, flag clearly (Grade F)
...
### PHASE 6: Edge Case & Boundary Sweep
...
- What if my assumptions about the user's context are wrong?
```
...
- [ ] Time-dependence (will this answer be wrong next week?)
- [ ] Cultural bias (does this assume Western/narrow perspective?)
- [ ] Survivorship bias (am I only looking at successes?)
- [ ] Selection bias (is my evidence cherry-picked?)
...
|---|---|---|
| Factual | Answer → Evidence → Caveats | "Paris is the capital of France..." |
| Analytical | Framing → Analysis → Conclusion | "The problem breaks into 3 parts..." |
| Decision | Recommendation → Reasoning → Risks | "I recommend option B because..." |
| Paradox | Dissolve paradox → Explain illusion → Conclusion | "The paradox arises from assuming X..." |
| Ethical | Key tension → Steelman both sides → Position + confidence | "This depends on whether you prioritize..." |
| Procedural | Steps → Rationale → Expected outcome | "First, verify the input; then..." |

---

### PHASE 8: Meta-Cognition & Process Audit (New)
*Pattern source: Advanced meta-cognitive patterns from DeepSeek R1, Claude Extended*

Review the thinking process itself:

```
META-AUDIT:
- Which phase consumed the most time? ___
- Did I catch an error during the process? [Y/N]
- Did I change my mind from initial intuition? [Y/N] — Why?
- Is there a reasoning pattern I tend to overuse in problems like this?
- What would I do differently next time for similar problems?
```

**Process Improvement Log:**

| Problem Type | Protocol Used | Outcome | Improvement for Next Time |
|---|---|---|---|
| [e.g., ethical dilemma] | Phases 1,3,4,6,7 | Solid | Should have run Phase 5 for evidence audit |
| [e.g., math problem] | Phases 1,2,4,7 | Strong | Decomposition could have been finer-grained |
...
|---|---|---|
| **Contradiction** | New information contradicts earlier conclusion | Return to Phase 3 |
| **Missing factor** | Realized a key variable was ignored | Return to Phase 2 |
| **False premise** | Initial assumption was wrong | Return to Phase 1 |
| **Overconfidence** | Evidence doesn't support confidence level | Return to Phase 5 |
...
5. Quick adversarial check (1 question)
...
Run all 8 phases. Don't skip the adversarial check. Document the meta-audit.

### Research Mode (Synthesis Tasks)
Weight Phase 5 (evidence evaluation) heavily. Run multiple passes of Phase 3 with different source priors. Use the Evidence Quality Grading table.

### Collaborative Mode (Working with Others)
After your own reasoning, present the conclusion with caveats and explicitly invite challenges. Use phrases like: "What am I missing?", "Where is this reasoning weakest?", "If you disagree, what's your best counterargument?"
...
| **Meta-cognitive audit** | DeepSeek R1 | After completing analysis |
| **Pre-mortem analysis** | Claude Extended | Before committing to high-stakes decisions |
...
| **False causality** | Assuming correlation implies causation | Ask: "Could there be a third factor?" |
| **Anchoring bias** | First number/estimate skews judgment | Generate independent estimates before comparing |
| **Dunning-Kruger effect** | Overconfidence in unfamiliar domains | Explicitly rate your domain expertise |
| **Sunk cost** | Continuing down a wrong path because time invested | "If I started over now, would I take this same path?" |
...
| Prompt | Purpose |
|---|---|
| **"What would I need to believe for the opposite to be true?"** | Expose hidden assumptions |
| **"What is the simplest possible explanation I haven't considered?"** | Occam's Razor check |
| **"If this were wrong, how would I find out?"** | Falsifiability test |
| **"What question am I not asking that I should be?"** | Blind spot discovery |
| **"What would change if I had 10x more information? What wouldn't?"** | Identify core certainties |
| **"What's the crux — the one disagreement that, if resolved, settles everything?"** | Focus on what matters |
| **"Am I solving a hard problem or a hard-seeming problem?"** | Differentiate actual vs. perceived complexity |
| **"What would my biggest critic say about this?"** | Adversarial preparation |
| **"How would I explain this to someone with no domain knowledge?"** | Simplify and clarify |
| **"If I'm right, what else should be true?"** | Test consistency of conclusion |
...
For procedural questions:  Lead with steps → then rationale → then expected outcome
```
...
This critical-thinking skill integrates with the following Quillan skills:

| Skill | Integration Point |
|---|---|
| **research-analysis** | Phase 5 (Evidence Evaluation) maps directly to the research-analysis Evidence Grading system |
| **technical-coding** | Phase 2 (Decomposition) aligns with technical debugging decomposition; Phase 6 catches edge cases |
| **swarm-inter-agent-orchestration** | Phases 3-4 (Hypothesis + Adversarial) inform agent routing decisions |
| **dev-team** | Phase 7 (Synthesis) output format aligns with dev-team delivery requirements |
...
**Final Quality Gate:**
Before delivering any output produced with this skill, verify:
- [ ] Problem correctly restated (Phase 1)
- [ ] Meaningful decomposition performed (Phase 2)
- [ ] At least 2 competing hypotheses considered (Phase 3)
- [ ] Adversarial check run against leading hypothesis (Phase 4)
- [ ] Evidence quality graded (Phase 5)
- [ ] Edge cases swept (Phase 6)
- [ ] Confidence explicitly stated (Phase 7)
- [ ] Process audited for improvement (Phase 8)
